// src/components/RecurringPaymentsList.jsx
...
export default function RecurringPaymentsList() {
  ...
  useEffect(() => {
    ...
    const q = query(
      collection(db, "users", user.uid, "recurringPayments"),
  ...
  const handleDelete = async (id) => {
    const ref = doc(db, "users", user.uid, "recurringPayments", id);
    await deleteDoc(ref);
  };
        ...
        >
          <div>
            ...
            <p className="text-sm text-gray-600 dark:text-gray-300">
              ₹{item.amount} | {item.category} | {item.frequency}
            </p>
            {item.startDate && (
